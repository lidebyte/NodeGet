...
最终磁盘位置 = `{static_path}/{path}/...`，URL 是 `/nodeget/static/{name}/...`，二者解耦。这意味着你可以给同一个磁盘目录起多个不同的 `name`（虽然唯一约束在 `name` 上，但你可以用新 `name` 指向旧 `path` 来做蓝绿切换）。
:::
...
如果某条静态配置的 `is_http_root` 为 `true`，则服务器的根路由 `/`（以及所有未匹配路由）会直接代理到该静态目录，**但不会覆盖 WebSocket、`/nodeget/*`、`/terminal` 等精确路由**。此时原有的默认占位 HTML 不再展示。
...
- `path` 字段按组件校验：每段允许 `[A-Za-z0-9_.-]`，允许 `/` 分隔多级子目录（如 `"sites/blog"`），长度 ≤ 512；不允许绝对路径、`..` 穿透、反斜杠、Windows 盘符前缀
