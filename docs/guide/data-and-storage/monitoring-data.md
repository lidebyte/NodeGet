...
具体来说，`全量动态监控数据` 在 SQLite / PostgreSQL 上都使用了 `JSONB` 作为数据储存，PostgreSQL 的 `JSONB` 储存效率要比
SQLite 好一些，数据压缩效能较高。虽然如此，利用 SQL 数据库储存 JSON 仍然是一种性价比较低的操作，储存效率远低于 `动态监控数据摘要`
，仅适合储存短期内的数据以供实时分析。
